...
//основная функция игрового цикла, отвечает за скорость змейки, обновление холста,отрисовку змейки и яблока
function gameLoop() {
...
//отвечает за обновление и отрисовку состояния змейки
function drawSnake() {
    snake.x += snake.dx;
    snake.y += snake.dy;
...
//отвечает за обработку столкновений змейки с границами игрового поля
...
//отвечает за обновление состояния игры после завершения текущей партии
function refreshGame() {
    const records = JSON.parse(localStorage.getItem("easyRecords")) || [];

    // Проверяем, входит ли текущий счет в 50 лучших результатов
...
function isInTopTen(currentScore, records) {
    if (records.length < 50) {
        // Если меньше 50 рекордов, то текущий счет всегда входит в топ-50
        return true;
    }
...
// установление случайной позиции для яблока
